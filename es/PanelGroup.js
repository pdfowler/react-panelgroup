...
var PanelGroup = (function(_React$Component) {
  ...
  function PanelGroup() {
    ...
    _this.getSizeDirection = function(caps) {
      if (caps) return _this.props.direction === "column" ? "Height" : "Width";
      else return _this.props.direction === "column" ? "height" : "width";
    };

    _this.handleResize = function(i, delta) {
      var tempPanels = _this.state.panels.slice();
      var returnDelta = _this.resizePanel(
        i,
        _this.props.direction === "row" ? delta.x : delta.y,
        tempPanels
      );
    ...
    _this.resizePanel = function(panelIndex, delta, panels) {
      ...
      var boundingSize =
        (_this.props.direction == "column"
          ? boundingRect.height
          : boundingRect.width) -
        _this.props.spacing * (_this.props.children.length - 1);
      ...
      if (panels[panelIndex].size < minsize) {
        ...
        if (panelIndex === 0)
          resultDelta = _this.resizePanel(panelIndex, _delta, panels);
        else resultDelta = _this.resizePanel(panelIndex - 1, -_delta, panels);
      }
      ...
      if (maxsize !== 0 && panels[panelIndex].size > maxsize) {
        ...
        if (panelIndex === 0)
          resultDelta = _this.resizePanel(panelIndex, -_delta2, panels);
        else resultDelta = _this.resizePanel(panelIndex - 1, _delta2, panels);
      }
      ...
      if (panels[panelIndex + 1].size < minsize) {
        ...
        if (panelIndex + 1 === panels.length - 1)
          resultDelta = _this.resizePanel(panelIndex, -_delta3, panels);
        else resultDelta = _this.resizePanel(panelIndex + 1, _delta3, panels);
      }
      ...
      if (maxsize !== 0 && panels[panelIndex + 1].size > maxsize) {
        ...
        if (panelIndex + 1 === panels.length - 1)
          resultDelta = _this.resizePanel(panelIndex, _delta4, panels);
        else resultDelta = _this.resizePanel(panelIndex + 1, -_delta4, panels);
      }

      // Iterate through left panel's snap positions
      for (var i = 0; i < panels[panelIndex].snap.length; i++) {
        if (
          Math.abs(panels[panelIndex].snap[i] - panels[panelIndex].size) < 20
        ) {
          var _delta5 = panels[panelIndex].snap[i] - panels[panelIndex].size;

          if (
            _delta5 !== 0 &&
            panels[panelIndex].size + _delta5 >=
              _this.getPanelMinSize(panelIndex, panels) &&
            panels[panelIndex + 1].size - _delta5 >=
              _this.getPanelMinSize(panelIndex + 1, panels)
          )
            resultDelta = _this.resizePanel(panelIndex, _delta5, panels);
        }
      }
      ...
      for (var _i = 0; _i < panels[panelIndex + 1].snap.length; _i++) {
        if (
          Math.abs(
            panels[panelIndex + 1].snap[_i] - panels[panelIndex + 1].size
          ) < 20
        ) {
          var _delta6 =
            panels[panelIndex + 1].snap[_i] - panels[panelIndex + 1].size;
          ...
          if (
            _delta6 !== 0 &&
            panels[panelIndex].size + _delta6 >=
              _this.getPanelMinSize(panelIndex, panels) &&
            panels[panelIndex + 1].size - _delta6 >=
              _this.getPanelMinSize(panelIndex + 1, panels)
          )
            resultDelta = _this.resizePanel(panelIndex, -_delta6, panels);
        }
      }
  ...
  PanelGroup.prototype.componentWillReceiveProps = function componentWillReceiveProps(
    nextProps
  ) {
  ...
  PanelGroup.prototype.render = function render() {
    ...
    var style = {
      ...
      ] = this.getPanelGroupMinSize(this.props.spacing)),
      (_container.display = "flex"),
      (_container.flexDirection = this.props.direction),
      (_container.flexGrow = 1),
      _container),
      panel: {
        flexGrow: 0,
        display: "flex"
    ...
    for (var i = 0; i < initialChildren.length; i++) {
      ...
      (_panelStyle[this.getSizeDirection()] = this.state.panels[i].size),
      (_panelStyle[this.props.direction === "row" ? "height" : "width"] =
        "100%"),
      (_panelStyle["min" + this.getSizeDirection(true)] =
        this.state.panels[i].resize === "stretch"
          ? 0
          : this.state.panels[i].size),
      (_panelStyle.flexGrow =
        this.state.panels[i].resize === "stretch" ? 1 : 0),
      (_panelStyle.flexShrink =
        this.state.panels[i].resize === "stretch" ? 1 : 0),
      (_panelStyle.display = "flex"),
      (_panelStyle.overflow = "hidden"),
      (_panelStyle.position = "relative"),
      _panelStyle);
      ...
      newChildren.push(
        React.createElement(
          Panel,
          _extends(
            { style: panelStyle, key: "panel" + i, panelID: i },
            metadata,
            {}
          ),
          initialChildren[i]
...
var Panel = (function(_React$Component2) {
  ...
  function Panel() {
    ...
    return (
      ...
      _this2)),
      (_this2.onResizeObjectLoad = function() {
        _this2.refs.resizeObject.contentDocument.defaultView.addEventListener(
          "resize",
          function() {
      ...
      (_this2.calculateStretchWidth = function() {
        if (_this2.props.onWindowResize !== null) {
          ...
          _this2.props.onWindowResize(_this2.props.panelID, {
            x: rect.width,
            y: rect.height
...
var Divider = (function(_React$Component3) {
  ...
  function Divider() {
    ...
    _this4.onMouseUp = function(e) {
      ...
      return _this4.onResizeComplete(_this4.props.panelID);
    };
    ...
    _this4.onResizeComplete = function(i) {
      return _this4.props.onResizeComplete(i);
    };
  ...
  Divider.prototype.componentDidUpdate = function componentDidUpdate(
    props,
    state
  ) {
